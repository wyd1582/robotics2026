...
纯文本公式：`reward = -(wrapped_angle**2 + 0.1*angular_velocity**2 + 0.001*torque**2)`。
三项分别惩罚偏离正上方、过快运动、控制努力；平方力矩不是实际电能。0.1 和 0.001 是任务设计权重，不是物理定律。
本课目标固定为正上方，所以观测不再附加目标坐标。它要学两个阶段：**摆起时积累动能，接近目标时减速并保持**。

注意：只最大化本步奖励会选零力矩，因为状态成本取自动作前。动作影响未来状态，必须考虑后续回报。
独立评测报告未折扣的 200 步奖励和；SAC 训练还含 gamma 折扣和熵项，这几个量不要混为一谈。
如果这段仍抽象，先做 [02A 六道直觉挑战](02A_SAC直觉与奖励设计挑战.ipynb) 的第 1–2 题，不需要先训练。
''')
code('''
...
全称是 **Soft Actor-Critic（SAC）**：Actor 提议动作；Critic 用真实奖励加后续价值估计来学习“这一步动作长期值不值”；Soft 指策略熵正则化，不表示电机动作轻柔。
最初 critic 的判断也不可靠，两个网络和实际交互一起迭代。Actor 不拟合专家标签，而是通过 critic 改进动作；部署评测通常只需要 Actor。
Replay 是过去真实交互的复用，不能把它当成新的环境步数；本课是 off-policy 在线交互学习，不等于能直接在任意固定离线数据上安全训练。

三个不同目标：**环境一步奖励**定义偏好；**critic loss**拟合带后续价值的目标；**actor loss**权衡价值与熵。另有自动熵系数的优化。
第 [02A](02A_SAC直觉与奖励设计挑战.ipynb) 3–4 题用小数字拆开这些概念，第 5–6 题检验反馈和奖励改动的实际行为。
''')
code('''
